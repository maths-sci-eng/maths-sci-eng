...
These can be estimated from the ogive plot. In our widget length example, we have
...
$$IQR=Q_3-Q_1$$
and is a measure of the spread of the data. For our widget lengths we have $IQR=$

A related plot is the *box plot*, which includes the LQ, UQ, Median and extreme values.
```{r boxplot}
y <- c(20.9,20.57,20.86,20.74,20.82,20.63,20.53,20.89,20.75,20.65,20.71,21.03,20.72,20.41,20.94,20.75,20.79,20.65,21.08,20.89,20.5,20.88,20.97,20.78,20.61,20.92,21.07,21.16,20.8,20.77,20.82,20.72,20.6,20.9,20.86,20.68,20.75,20.88,20.56,20.94)
boxplot(y, main = "Widget lengths", xlab = "length (mm)", ylab = "Sample of 40 widgets", horizontal = TRUE)
```

More generally, we define the $k^\text{th}$ *percentile* $p$ as the value for which $k$% of the data falls before $p$. For example, the $25^{\text{th}}$ percentile is the same thing as $Q_1$.
