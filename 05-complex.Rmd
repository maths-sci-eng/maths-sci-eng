...
$$x^2-4=0$$
and so must have two roots. Theorem \ref@(thm:FTA) therefore implies that there are $n$ solutions to taking the $n^{th}$ root of *any* number $w$, although these could be complex, since we are equivalently looking for the roots of the polynomial
$$z^n-w=0.$$
Furthermore, it turns out that these roots will be distinct (no repeated roots). The easiest way to find these complex roots is using the exponential form.
